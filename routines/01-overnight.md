...
3. **Entry scan (Technical analyst pass):** For each pair in universe not currently open, fetch 1H + 4H OHLCV and check entry conditions per `strategy.md` (rules 1, 2, 2a, 3, 4, 4a, 5, 5a, 5b, 6, 6a, 7, 8). Compute indicators in-line. Record per-pair Pass/Fail in research_log under `### Technical` subsection.
   - **Indicator warm-up (added 2026-06-11):** request **720 bars** of 4H history (Kraken's per-call max). The 4H 50-EMA needs ≥ 200 bars of warm-up to converge — a 50-EMA seeded from ~60 bars carries $400–500 of error on BTC, enough to flip rule 3 either way (this ambiguity forced the 06-11 EOD entry deferral). Never adjudicate a marginal rule 3 on a short-warm-up EMA; if < 150 4H bars exist for a pair, log the rule-3 result as LOW-CONFIDENCE.
