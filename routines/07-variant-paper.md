...
- **For the current (most recent) bar's rule evaluation, run `python scripts/indicators.py` and treat its converged table as authoritative** (it also prints the regime 5a/SBD verdict and 2×ATR stops). Historical wakes inside the replay window still require computing indicators from the fetched bars — do that with the same conventions the script documents (SMA-seeded EMA over full history, Wilder RSI/ATR), never from a short warm-up.
